...
export default function AdminDistricts() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const payload = {
        division_id: Number(form.division_id) || form.division_id, // cast if numeric
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <h2 className="text-2xl font-semibold mb-4">Manage Districts</h2>
          ...
          <tbody>
            {items.map((it) => (
              <tr key={it.id} className="border-b">
                ...
                <td className="p-2">{it.division?.name_en || "-"}</td>
